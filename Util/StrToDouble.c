...
/******************************************************************************
 * Equivalent to strtod() but with simplified error reporting.
...
{ /* StrToDouble(const char *, double *) */
  ...
  /* strtod() returns error status in 'errno' but we must leave it
   * unchanged. */
  SavedErrNo = errno;

  /* Do the conversion. */
  errno = 0;
  NewVal = strtod(Str, &End);
  ErrNo = errno;
   ...
   *    this case strtod() will not have used all of 'Str' (i.e. (*End
   *    != '\0')) and we return EINVAL.
   *
   * 2) 'Str' is out of range.  In this case, strtod() sets 'errno' to
  ...
  errno = SavedErrNo;
  return ErrNo;
} /* StrToDouble(const char *, int, double *) */
